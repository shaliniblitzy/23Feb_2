"""Alembic runtime environment for the Notification Service.
...
the Notification Service's private PostgreSQL database (``notification_db``).

Runtime contract:
    * The ``POSTGRES_URL`` environment variable MUST be set. Its value is the
      PostgreSQL connection URL in either of these forms::

          postgresql://user:pass@host:port/notification_db
          postgresql+psycopg://user:pass@host:port/notification_db

      The first form (bare ``postgresql://``) is rewritten to use the psycopg
      v3 SQLAlchemy dialect (``postgresql+psycopg://``) because the service
      pins ``psycopg[binary,pool]>=3.1.18`` (NOT psycopg2). See
      ``services/notification-service/requirements.txt``.
    * ``target_metadata`` is intentionally ``None``. The Notification
      Service's runtime uses pure psycopg3 with ``AsyncConnectionPool`` (no
      SQLAlchemy ORM). Migrations use imperative ``op.create_table()`` /
      ``op.create_index()`` calls rather than autogenerated metadata
      comparisons. See ``./README.md``.

Exit codes (relevant for K8s Job / initContainer observability):
    * ``2`` -- ``POSTGRES_URL`` environment variable is unset or empty. This
      is a configuration error and is distinct from migration failures so
      operators can filter the two with a simple ``$? -eq 2`` check.
...
References:
    * AAP Section 0.4.4   -- notification_db schema (notification_log,
                             delivery_attempts, templates, user_channel_prefs)
...
#: ``alembic.ini`` declares ``sqlalchemy.url =`` (empty) precisely because the
#: connection URL must be resolved here, never persisted in source or config.
_DB_URL_ENV_VAR: Final[str] = "POSTGRES_URL"

#: Environment variable for optional Alembic log-level override. Operators can
#: set ``ALEMBIC_LOG_LEVEL=DEBUG`` in CI to surface verbose diagnostic output
#: without committing a change to ``alembic.ini``.
_LOG_LEVEL_ENV_VAR: Final[str] = "ALEMBIC_LOG_LEVEL"

#: Default log level if ``ALEMBIC_LOG_LEVEL`` is not set. WARNING keeps stderr
#: quiet during routine ``alembic upgrade head`` runs while still surfacing
#: actionable warnings.
...
#: 'psycopg2'`` deep inside its plugin loader.
...
#: opposed to Alembic's ``alembic.runtime.migration`` logger which reports on
#: revision execution). The ``alembic.env`` qualname slots cleanly under the
#: ``alembic`` parent logger configured by ``fileConfig`` below.
...
# ``getattr`` resolves "DEBUG" / "INFO" / "WARNING" / "ERROR" / "CRITICAL" to
# their integer constants; an unknown name silently falls back to WARNING so
# a typo in CI does not silently produce a flood of DEBUG output.
...
def _normalize_database_url(raw_url: str) -> str:
    ...
    The Notification Service pins ``psycopg[binary,pool]>=3`` (see
    ...
    ``postgresql://...`` URLs (the libpq default); this helper rewrites them
    so SQLAlchemy picks the correct dialect.

    The function is intentionally narrow:

    * It does NOT validate host/port/database parts -- that is SQLAlchemy's
      job and producing a duplicate validator here would create drift.
    * It does NOT URL-encode credentials -- callers are responsible for
      providing a well-formed URL and must encode special characters in the
      password according to RFC 3986.
    * It DOES reject the legacy ``postgresql+psycopg2://`` scheme with an
    ...
    Raises:
        RuntimeError: If the caller supplied a ``postgresql+psycopg2://``
            ...
            not function with it; the failure must be explicit, not silent.
    """
    if _REJECTED_PSYCOPG2_SCHEME_PATTERN.match(raw_url):
        raise RuntimeError(
            "POSTGRES_URL uses the legacy 'postgresql+psycopg2://' driver, "
            "but the Notification Service pins 'psycopg[binary,pool]>=3' in "
...
def _resolve_database_url() -> str:
    ...
    This function is called once during Alembic env.py import. A missing env
    var is treated as a fatal startup error (AAP R-19, R-25): the process
    writes a clear remediation message to stderr and exits with code ``2`` so
    that Kubernetes Job / initContainer operators can distinguish a
    missing-config failure (exit 2) from a migration execution failure
    (exit 1, raised by Alembic / SQLAlchemy / psycopg).
    ...
    if not raw_url:
        sys.stderr.write(
            ...
            "Set POSTGRES_URL to the notification_db connection URL, e.g.:\n"
            "  POSTGRES_URL=postgresql://user:pass@host:5432/notification_db\n"
            "See services/notification-service/.env.example for the template.\n"
        )
        sys.exit(2)
...
# happen before Alembic prints any banner output, which is the user-friendly
# behavior for ``alembic ...`` invocations.
...
# The Notification Service uses pure psycopg3 + Pydantic at runtime; there is
# no SQLAlchemy MetaData() object to diff against. All schema is defined
# imperatively in ``versions/<revision>.py`` via ``op.create_table()`` and
# ``op.execute()`` calls. Leaving this None disables ``--autogenerate``;
# revisions must be written by hand, which is the intended workflow:
#
#   * ``alembic revision -m "create notification_log"`` produces an empty
#     revision file under ``versions/``;
#   * the engineer fills in ``upgrade()`` and ``downgrade()`` with explicit
#     ``op.create_table(...)`` / ``op.drop_table(...)`` calls;
#   * the result is reviewed and merged like any other source code change.
#
# DO NOT replace this with a ``MetaData()`` object -- it is a deliberate
# architectural decision that reconciles "Alembic uses SQLAlchemy under the
# hood" with "the runtime service code does not import SQLAlchemy".
...
def run_migrations_offline() -> None:
    ...
    Configuration notes:
        * ``literal_binds=True`` inlines parameter values into the generated
          SQL because the resulting script is meant for human review and for
          replay through ``psql -f``; bound-parameter placeholders make the
          script unusable in those contexts.
        * ``dialect_opts={"paramstyle": "named"}`` pairs with literal_binds
          to produce ``:name`` style placeholders for any binds Alembic
          cannot literalize (e.g., user-defined Python objects); these are
          rare for the Notification Service's straightforward DDL.
        * ``compare_type=True`` and ``compare_server_default=True`` enable
          type / default diff detection if ``--autogenerate`` is ever turned
          on later; they are harmless for the manual-revision workflow.
        * ``render_as_batch=False`` because PostgreSQL supports true ALTER
          statements; batch mode is a SQLite workaround that produces noisy
          ``CREATE TABLE _alembic_tmp_<name>`` recipes.
    """
    context.configure(
        url=_database_url,
...
def run_migrations_online() -> None:
    ...
    that brings the notification_db schema up to date before the service
    pods come online.
    ...
    Notes:
        * We use ``pool.NullPool`` -- migration runs are short-lived,
          ...
          well-documented issue where DDL issued through a pooled connection
          can appear rolled back if the pool reclaims the connection
          mid-migration; NullPool is the migration community's standard
          recommendation for this reason.
        * SQLAlchemy + psycopg3 fully supports synchronous engines; we do
          NOT use the async engine here because Alembic's upstream execution
          model is synchronous (``context.run_migrations()`` calls into
          ``MigrationContext._do_run_migrations`` synchronously). Async
          support requires a separate ``async def run_migrations_online()``
          + ``asyncio.run()`` wrapper, which we do not need for
          PostgreSQL-only DDL.
        * ``transaction_per_migration=True`` wraps each revision's
          upgrade / downgrade in its own transaction. This is critical for
          forward-only safety: if one of several queued revisions fails,
          only that revision's changes are rolled back; previously-applied
          revisions remain durable. The default (``False``) wraps the
          entire ``alembic upgrade head`` in a single transaction, which
          means a failure on revision N rolls back N-1, N-2, ... too.
    """
    ...
    connectable = engine_from_config(
        ...
        # this in 1.13+ but we pass it explicitly for forward compatibility
...
# ``context.is_offline_mode()`` is True when the Alembic CLI was invoked with
# ``--sql`` (i.e., the operator wants the migration emitted to stdout as
# raw DDL rather than executed against a live database). Any other invocation
# uses the online path. There is no third mode.
