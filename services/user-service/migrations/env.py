"""Alembic runtime environment for the User Service.
...
the User Service's private PostgreSQL database (``user_db``).

Runtime contract:
    * The ``POSTGRES_URL`` environment variable MUST be set. Its value is
      the PostgreSQL connection URL in either of these forms::

          postgresql://user:pass@host:port/user_db
          postgresql+psycopg://user:pass@host:port/user_db

      The first form (bare ``postgresql://``) is rewritten to use the
      psycopg v3 SQLAlchemy dialect (``postgresql+psycopg://``) because
      the service pins ``psycopg[binary,pool]>=3.1`` (NOT psycopg2). See
    ...
    * ``target_metadata`` is intentionally ``None``. Schema is defined
      ...
      every DDL change in a service that handles PII (email, phone,
      addresses, DOB) must be reviewed line-by-line for data-protection
      implications.
    * ``version_table`` is set to ``alembic_version_user_service`` (NOT
      ...
      development without colliding on a shared version table. This is
      the ONLY substantive behavioral difference from
      ``services/notification-service/migrations/env.py``; everything
      else (imports, regex patterns, dispatch logic, exit-code semantics)
      is intentionally kept near-identical for monorepo consistency. The
      same name is documented as ``MIGRATIONS_TABLE`` in
      ``../.env.example`` so operators have a single authoritative source
      of truth for the version-table identifier.
...
Exit codes (relevant for K8s Job / initContainer observability):
    * ``2`` -- ``POSTGRES_URL`` environment variable is unset or empty.
      ...
      failures so operators can filter the two with a simple
      ``$? -eq 2`` check.
...
References:
    * AAP Section 0.4.4   -- user_db schema (4 user-domain tables:
                             ``users``, ``user_profiles``,
                             ``user_preferences``, ``user_addresses``;
                             plus 1 transactional outbox table)
    ...
    * AAP R-6             -- Database per service strictly enforced;
                             ``user_db`` is owned exclusively by the User
                             Service
...
#: config. The User Service treats this rule with extra care: ``user_db``
#: stores PII -- email, phone, postal addresses, and date-of-birth -- so a
#: leaked URL would be a Sev-1 privacy incident.
...
#: with multiple services (auth_db, user_db, payment_db, notification_db,
#: etc.) would otherwise collide on the default ``alembic_version`` table
#: when sharing a single Postgres instance. Per the migrations folder
#: spec mandate and per the ``MIGRATIONS_TABLE`` setting documented in
#: ``../.env.example``.
#:
...
#: scenario AND conflicting with the ``MIGRATIONS_TABLE=
#: alembic_version_user_service`` setting in ``../.env.example`` that
#: operators have set as authoritative.
_VERSION_TABLE_NAME: Final[str] = "alembic_version_user_service"
...
#: on revision execution). The ``alembic.env`` qualname slots cleanly
#: under the ``alembic`` parent logger configured by ``fileConfig`` below.
...
def _normalize_database_url(raw_url: str) -> str:
    ...
    The User Service pins ``psycopg[binary,pool]>=3.1`` (see
    ...
    * It DOES reject the legacy ``postgresql+psycopg2://`` scheme with an
    ...
    if _REJECTED_PSYCOPG2_SCHEME_PATTERN.match(raw_url):
        raise RuntimeError(
            ...
            "but the User Service pins 'psycopg[binary,pool]>=3' in "
...
def _resolve_database_url() -> str:
    ...
    if not raw_url:
        sys.stderr.write(
            ...
            "Set POSTGRES_URL to the user_db connection URL, e.g.:\n"
            "  POSTGRES_URL=postgresql://user:pass@host:5432/user_db\n"
            "See services/user-service/.env.example for the template.\n"
        )
        sys.exit(2)
...
# the intended workflow for the user-service domain because every DDL
# change must be reviewed line-by-line for PII / data-protection
# implications: ``user_db`` stores email addresses, phone numbers, postal
# addresses, and date-of-birth values for every registered user (AAP
# Section 0.4.4). A missing ORM-model column would silently drop a PII
# attribute on apply -- unacceptable for this domain.
#
...
# the hood" with "the user-service domain requires hand-reviewed DDL".
# Future contributors who attempt to wire in ORM models for autogenerate
# convenience should consult ``./README.md`` and the parent service's
# README "Toolchain" section for the rationale.
...
def run_migrations_offline() -> None:
    ...
    No connection is opened; the URL is used only for dialect selection
    so Alembic can render PostgreSQL-flavored DDL (e.g., ``TIMESTAMPTZ``,
    ``JSONB``, ``ON CONFLICT``) for the user-domain tables.

    Configuration notes:
        * ``version_table=_VERSION_TABLE_NAME`` ensures the
          offline-emitted SQL targets ``alembic_version_user_service``
          (NOT the default ``alembic_version``). This matters for
          ``--sql`` review scripts: an operator who pipes the script
          through ``psql`` against a shared dev Postgres must not
          collide with sibling services' version tables.
        ...
        * ``dialect_opts={"paramstyle": "named"}`` pairs with
          ...
          objects); these are rare for the User Service's straightforward
          DDL.
        * ``compare_type=True`` and ``compare_server_default=True`` enable
          ...
          workflow.
        * ``render_as_batch=False`` because PostgreSQL supports true
          ALTER statements; batch mode is a SQLite workaround that
          produces noisy ``CREATE TABLE _alembic_tmp_<name>`` recipes.
    """
    context.configure(
        url=_database_url,
...
def run_migrations_online() -> None:
    ...
    that brings the user_db schema up to date before the service pods
    come online.
    ...
    Notes:
        * ``version_table=_VERSION_TABLE_NAME`` ensures Alembic creates /
          reads / writes ``alembic_version_user_service`` instead of the
          default ``alembic_version``. Multiple services (auth-service,
          user-service, payment-service, notification-service, etc.) can
          share a single Postgres instance during local development
          without clashing on a shared bookkeeping table.
        ...
        * SQLAlchemy + psycopg3 fully supports synchronous engines; we do
          ...
          do not need for PostgreSQL-only DDL. The User Service runtime
          uses async SQLAlchemy + async psycopg3 for request handling,
          but migrations always run through the sync engine.
        * ``transaction_per_migration=True`` wraps each revision's
          ...
          back N-1, N-2, ... too -- unacceptable when user-domain tables
          may be partially populated by intermediate revisions during a
          multi-step expand-contract migration sequence.
    """
